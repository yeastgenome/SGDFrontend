  ...
  render: function () {
    ...
    var controlsNode = (
      <RadioSelector
        ...
        initialActiveElementKey={this.state.activeDataKey}
        onSelect={_onSelect}
      />
    );

    // get the data that corresponds to the active element
    var active = _.findWhere(this.props.data, {
      key: this.state.activeDataKey,
    });
    ...
    var _colorScale = (d) => {
      return d.isRoot ? '#DF8B93' : '#18AB2F';
    };
    var _helpText =
      "Features that are annotated to the ‘root’ terms are considered ‘unknown’ and are shown in red.  More information on GO and GO slims can be found on SGD’s <a target='_blank' href='https://sites.google.com/view/yeastgenome-help/function-help/gene-ontology-go'>GO help page</a>.  Please use the <a href='http://www.yeastgenome.org/cgi-bin/GO/goSlimMapper.pl'>GO Slim Mapper</a> to obtain the GO data summarized in these graphs.";
    return (
      <div className="toggle-bar-chart">
        <h3 style={{ position: 'absolute', top: '1rem', right: '1rem' }}>
          <HelpIcon isInfo={true} orientation="left" text={_helpText} />
        </h3>
        <div style={{ marginRight: '2rem' }}>{controlsNode}</div>
        <BarChart
          data={activeData}
