...
var Parset = createReactClass({
  ...
  render: function () {
    ...
    var _polygonString = `${x1[0]},${LINE_HEIGHT} ${x1[1]},${LINE_HEIGHT} ${
      x2[1]
    },${HEIGHT - LINE_HEIGHT} ${x2[0]},${HEIGHT - LINE_HEIGHT}`;

    var labelNode = this._getLabelNode();
    var polygonNode = !this.props.isVisible ? null : (
      <polygon points={_polygonString} fill="#DEC113" opacity={0.5} />
    );
    var x1LineNode = this._getX1LineNode();
    var x2LineNode = this._getX2LineNode();

    return (
      <div className="parset" style={{ height: HEIGHT, position: 'relative' }}>
        {labelNode}
  ...
  _getLabelNode: function () {
    ...
    if (this.props.contigDisplayName && this.props.contigHref) {
      anchorNode = (
        <a href={this.props.contigHref}>{this.props.contigDisplayName}</a>
      );
    }
    return (
      <h3
      ...
      >
        S288C Coordinates: {anchorNode} {this.props.text}
      </h3>
    );
  },
