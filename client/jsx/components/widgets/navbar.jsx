...
const Navbar = createReactClass({
  ...
  render: function () {
    ...
    if (this.props.title) {
      var titleNode = this.props.title.href ? (
        <a
          href={this.props.title.href}
          dangerouslySetInnerHTML={{ __html: this.props.title.name }}
        />
      ) : (
        <span dangerouslySetInnerHTML={{ __html: this.props.title }} />
      );
      listElements.unshift(
        <li key="titleNode" id="nav-title">
          {titleNode}
        </li>
      );
    }
    ...
    if (this.state.backToTopVisible) {
      backToTopNode = (
        <a
          onClick={this._onClickToTop}
          href="#"
          className="back-to-top"
          style={{ position: 'fixed', display: 'inline', zIndex: 1 }}
        >
          Back to Top
        </a>
      );
