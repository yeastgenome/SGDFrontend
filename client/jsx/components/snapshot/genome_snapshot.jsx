...
module.exports = createReactClass({
  ...
  toggleShowChromosomes: function (e) {
    ...
    this.setState((prevState) => ({
      showChromosomes: !prevState.showChromosomes,
    }));
  },
  ...
  render: function () {
    var _buttonText = this.state.showChromosomes ? (
      <span>
        Show by Genome&nbsp;<i className="fa fa-angle-up"></i>
      </span>
    ) : (
      <span>
        Show by Chromosome&nbsp;<i className="fa fa-angle-down"></i>
      </span>
    );
    var buttonNode = (
      <a
        onClick={this.toggleShowChromosomes}
        className="button small chromosome-toggle"
        role="button"
      >
        {_buttonText}
      </a>
    ...
    if (this.state.showChromosomes) {
      ...
      barNodes = _.map(this.props.data.chromosomes, (c) => {
        var chromosomeLabelNode = (
          <div className="clearfix chromosome-label-text">
            <h3>
              {c.display_name}&nbsp;-&nbsp;<a href={c.link}>Details</a>
            </h3>
        ...
        return [
          chromosomeLabelNode,
          this._getBarChart(c.features, _maxFeatures),
        ];
      });
    } else {
      let _maxFeatures = d3.max(this.props.data.combined, (d) => {
        return d.value;
      });
  ...
  _getBarChart: function (data, maxY) {
    ...
    var legendElements = _.map(data[0].nestedValues, (entry, i) => {
      ...
      var _percent =
        totalOrfs > 0 ? Math.round((entry.value / totalOrfs) * 100) : 'n/a';
      return {
    ...
    var legendNode = (
      <div style={{ marginLeft: `${_labelRatio * 100}%`, fontSize: 12 }}>
        <Legend
          elements={legendElements}
          labelText={`${totalOrfs.toLocaleString()} ORFs`}
        />
      </div>
    );
