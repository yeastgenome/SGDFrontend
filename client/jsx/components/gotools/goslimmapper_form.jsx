...
const GoSlimMapper = createReactClass({
        ...
        <div style={{ textAlign: 'top' }}>
          <h3>
            <strong style={{ color: 'red' }}>
              Your gene list has been saved in the memory. Please pick a GO Slim
              Set, refine the Slim Terms, and Submit the form.
            </strong>
          </h3>
  ...
  onSubmit(e) {
    ...
    if (fromTools == 0) {
	  ...
	  warningMsg = warningMsg + "<strong>" + gene + "</strong>:<ul>";
		...
		warningMsg = warningMsg + "<li><input type='radio' id='" + geneID + "' name='" + geneID + "' value='" + geneObj['sgdid'] + "' onclick=\"window.alert('hello alias');\"/><label for='"+ geneID + "'>an alias name for " + display_name + "</label></li>";
            }
	    else {
              warningMsg = warningMsg + "<li><input type='radio' id='" + geneID + "' name='" + geneID + "' value='" + geneObj['sgdid'] + "' onclick=\"window.alert('hello std name');\"/><label for='"+ geneID + "'>a standard gene name for " + display_name + "</label></li>";
            }
          }
  ...
  onChange4slimtype(e) {
    this.setState({ text: e.target.value, slimType: e.target.value });
  },
...
e-help/analyze-help/go-slim-mapper?authuser=0' target='help_win'>GO Slim</a> terms of your choice.</h3>";
  ...
  tableSaveOptions(htmlUrl, termsUrl, tabUrl, inputUrl, slimTermUrl) {
    return (
      ...
      inputUrl +
      " target='infowin2'>Your Input List of Genes</a> | <a href=" +
      slimTermUrl +
      " target='infowin2'>Your GO Slim List</a></h3>"
    );
  },
});
