...
const GoTermFinder = React.createClass({
	...
	getGeneBox() {
			...
			<h3><strong>Enter Gene/ORF names</strong> (separated by a return or a space):
                        <textarea ref='genes' onChange={this._onChange} name='genes' rows='2' cols='150'></textarea>
	...
	getGene4bgBox() {
			...
			<strong style={{ color: 'red'}}>OR</strong> <strong>Upload a file of Gene/ORF names</strong> (.txt or .tab format):
                        <p><input className="btn btn-default btn-file" type="file" name='uploadFile' onChange={this.handleFile4bg} accept="image/*;capture=camera"/></h3>
                        </div>);
	...
	getOptionalInput() {
		...
		var _pvalueElements = [<option value='0.01' selected='selected'>0.01</option>];
		       ...
		       <h3>The default p-value cutoff is 0.01. Pick a different p-value below:</h3>
