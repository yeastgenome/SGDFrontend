...
const SequenceToggler = createReactClass({
      ...
      customRetrievalNode = (
        <ul className="button-group radius">
          <a
            ...
            href={'/seqTools?seqname=' + this.props.locusFormatName}
          >
          ...
          <ul className="button-group radius">
            <li>
              <DownloadButton
                ...
                url="/download_sequence"
                extension=".fsa"
                params={_downloadParams}
              />
            </li>
          </ul>
          {customRetrievalNode}
  ...
  _getDropdownNode: function () {
    ...
    return (
      <DropdownSelector
        ...
        onChange={this._handleChangeSequence}
      />
    );
  },
  ...
  _formatActiveSequenceTextNode: function () {
    ...
    if (this.props.showSequence) {
      ...
      if (this._canColorSubFeatures()) {
        ...
        var _featureTypes = _.uniq(
          _.map(this.props.subFeatureData, (f) => {
            return f.class_type;
          })
        );
  ...
  _getSimpleSequenceNode: function (sequence) {
    ...
    var maxLabelLength = (lineArr.length * LETTERS_PER_LINE + 1).toString()
      .length;
  ...
  _getComplexSequenceNode: function (sequence) {
    ...
    return _.map(chunked, (c, i) => {
      i++;
      var sp =
        i % LETTERS_PER_CHUNK === 0 && i % LETTERS_PER_LINE !== 0 ? ' ' : '';
      var cr = i % LETTERS_PER_LINE === 0 && i > 1 ? '\n' : '';
      var str = c + sp + cr;
      var _classType = this._getSubFeatureTypeFromIndex(i - offset);

      var labelNode =
        (i - 1) % LETTERS_PER_LINE === 0 ? (
          <span style={{ color: '#6f6f6f' }}>{`${Array(
            maxLabelLength - i.toString().length
          ).join(' ')}${i} `}</span>
        ) : null;
      ...
      return (
        <span
          key={`sequence-car${i}`}
          style={{ color: subFeatureColorScale(_classType) }}
        >
          {labelNode}
          {str}
        </span>
  ...
  _canColorSubFeatures: function () {
    ...
    if (
      this.props.subFeatureData.length <= 1 ||
      this.state.activeSequenceType === 'coding_dna' ||
      this.state.activeSequenceType === 'protein'
    ) {
      return false;
    }

    // no overlaps
    var _allTracks = _.uniq(
      _.map(this.props.subFeatureData, (f) => {
        return f.track;
      })
    );
  ...
  _getActiveSequence: function () {
    return _.findWhere(this.props.sequences, {
      key: this.state.activeSequenceType,
    });
  },
});
