...
module.exports = class VariantViewerStore {
  ...
  setVisibleStrainIds(_visibleStrainIds, cb) {
    ...
    if (_visibleStrainIds.indexOf(REFERENCE_STRAIN_ID) < 0)
      _visibleStrainIds.push(REFERENCE_STRAIN_ID);
    _visibleStrainIds = _.sortBy(_visibleStrainIds, (d) => {
      return d.id;
    });
  ...
  setIsProteinMode(_isProteinMode) {
  ...
  getLociData() {
    return query === '' ? allLociData : filteredlociData;
  }

  getLocusData() {
    return visibleLocusData;
  }

  getClusteredStrainData() {
    return clusteredStrainData;
  }
  ...
  getStrainMetaData() {
    return staticStrainMetadata.strains;
  }
  ...
  getNumVisibleLoci() {
    return Math.min(
      totalLoci,
      Math.round(SCROLL_CONTAINER_HEIGHT / heatmapZoom)
    );
  }
  ...
  fetchInitialData(cb) {
    ...
    this.setVisibleStrainIds(
      strainMetaData.map((d) => {
        return d.id;
      })
    );
  ...
  clusterStrains(cb) {
    ...
    if (
      query === '' &&
      visibleStrainIds.length === staticStrainMetadata.strains.length
    ) {
      clusteredStrainData = staticStrainMetadata.clusterData;
      strainClusterIndexes = this.getStrainIndexes(clusteredStrainData);
      cb(null);
    } else {
      var visibleStrainMetaData = visibleStrainIds.map((d) => {
