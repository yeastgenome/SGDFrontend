$(document).ready(function() {
	...
	// var headers = ["Evidence ID", "Analyze ID", "", "Gene", "Gene Format Name", "Gene Ontology Term", "Gene Ontology Term ID", "Qualifier", "Aspect", "Method", "Evidence", "Source", "Assigned On", "Reference", "Relationships"]     ;
  	...
  	// GO-CAMs -- embed the GO Consortium <go-gocam-viewer>, driven by the model
  	// id(s) for this locus. Mirrors the LSP GO-CAM display (see
  	// functional_networks.js, redmine 6631). Hide the section if the gene has none.
  	$.getJSON('/redirect_backend?param=locus/' + locus['id'] + '/go_cams', function(models) {
...
// Render the GO-CAM viewer for this locus. Mirrors fn_render_gocams() in
// functional_networks.js: a pull-down appears only when the gene has more than
// one model, and the <go-gocam-viewer> is replaced (not mutated) on each switch
// so every model gets a clean first-render fit.
