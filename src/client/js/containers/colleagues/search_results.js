...
module.exports = React.createClass({
  render: function() {
    var rows = this.props.results.map(function(r, i) {
      return (
        <div className='colleaguesSearchResult column row' key={`cSearchRes${i}`}>
          ...
          {r.email ? <span><i className='fa fa-envelope'></i> {r.email} | </span> : null}
          {r.work_phone ? <span><i className='fa fa-phone'></i> {r.work_phone}</span> : null}
