...
class HelpersTest(unittest.TestCase):
    ...
    @mock.patch('src.helpers.DBSession')
    def test_reference_side_effect(*args, **kwargs):
        import pdb;
        pdb.set_trace()
        ...
        if len(args) == 1 and str(args[0]) == "<class 'src.models.ReferenceUrl'>":
            reference = factory.ReferenceUrlFactory()
            return MockQuery(reference)
        if len(args) == 1 and str(args[0]) == "<class 'src.models.DatasetReference'>":
            datasetref = factory.DatasetReferenceFactory()
            return MockQuery((datasetref.display_name, datasetref.url_type, datasetref.obj_url))
        elif len(args) == 1 and str(args[0]) == "<class 'src.models.ReferenceDocument'>":
            refdoc = factory.ReferencedocumentFactory
            return MockQuery((refdoc.summary_id, refdoc.html))
        elif len(args) == 1 and str(args[0]) == "<class 'src.models.Referencetype'>":
            reftype = factory.ReferencetypeFactory()
            return MockQuery((reftype))
        ...
        elif len(args) == 1 and str(args[0]) == "<class 'src.models.Referenceauthor'>":
            refauthor = factory.ReferenceauthorFactory()
            return MockQuery((refauthor))
            ...
            # DBSession.query(Referencedocument.text).filter_by(reference_id=self.dbentity_id, document_type="Abstract").one_or_none()
