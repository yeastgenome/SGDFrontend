namespace :deploy do
  ...
  task :config do
    on roles(:app), in: :sequence do
      variables = "'"
      ["NEX2_URI", "S3_ACCESS_KEY", "S3_SECRET_KEY", "S3_BUCKET", "GOOGLE_CLIENT_ID", "ES_URI", "ES_INDEX_NAME", "GOOGLE_CAPTCHA_SECRET", "PUSHER_APP_ID", "PUSHER_KEY", "PUSHER_SECRET", "CACHE_URLS", "WORKER_LOG_FILE", "BATTER_URI"].each do |k|
